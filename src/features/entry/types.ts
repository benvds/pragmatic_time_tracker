/**
 * Time Entry Types
 * Based on existing entry form structure with localStorage persistence
 */

export interface TimeEntry {
  id: string;           // Unique identifier (UUID v4)
  date: string;         // ISO date format (YYYY-MM-DD)
  description: string;  // Work description (required, min 3 chars)
  project: string;      // Project name (required, min 2 chars)
...
// Form data interface matching existing + new date field
export interface TimeEntryFormData {
  date: string;         // NEW: ISO date field
...
export interface ITimeEntryService {
   ...
   */
  create(request: CreateTimeEntryRequest): Promise<ServiceResponse<TimeEntry>>;
   ...
   */
  getForMonth(year: number, month: number): Promise<ServiceResponse<TimeEntry[]>>;
   ...
   */
  update(id: string, updates: UpdateTimeEntryRequest): Promise<ServiceResponse<TimeEntry>>;
